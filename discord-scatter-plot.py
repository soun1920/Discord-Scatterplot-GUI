...
def run_code():
    ...
    for dir in os.listdir(root):
        ...
        if os.path.isdir(dir_path):
            ...
            try:
                with open(
                    os.path.join(dir_path, "messages.json"), "r", encoding="utf-8"
                ) as json_file:
                    json2 = json.load(json_file)
                    for message in json2:
                        time = datetime.fromisoformat(message["Timestamp"]) + timedelta(
                            hours=9
                        ).astimezone(JST)
                        dates.append(time)
    ...
    if not dates:
        messagebox.showerror(
            "Error", "Wrong folder selected. Make sure you selected the messages folder"
        )
        return
...
def create_scatterplot(days, times, yourNameHere):
    ...
    if file_type_var.get() == "PNG":
        ...
        messagebox.showinfo(
            "Success",
            "PNG file created. Look for out.png in the same folder you ran this exe file in",
        )
    else:  # file_type_var.get() == "SVG"
        plt.savefig("out.svg", bbox_inches="tight", pad_inches=0.3)
        messagebox.showinfo(
            "Success",
            "SVG file created. Look for out.svg in the same folder you ran this exe file in",
        )
...
header_label = customtkinter.CTkLabel(
    root_window, text="Discord Scatter plot maker", font=("Arial", 24, "bold")
)
header_label.grid(pady=10)  # Add vertical padding

label = customtkinter.CTkLabel(
    root_window,
    text="Please enter the username you'd like displayed on the final image",
)
...
label = customtkinter.CTkLabel(
    root_window, text="Please select your messages folder here"
)
label.grid(pady=10)  # Add vertical padding

select_button = customtkinter.CTkButton(
    root_window, text="Select Folder", command=select_directory
)
select_button.grid(pady=0)  # Add vertical padding

selected_folder_label = customtkinter.CTkLabel(
    root_window, text=""
)  # Add a new label for the selected folder
...
plot_type_dropdown = customtkinter.CTkOptionMenu(
    root_window,
    values=["Scatterplot", "Heatmap"],
    command=update_dark_mode_visibility,
...
dark_mode_checkbox = customtkinter.CTkCheckBox(
    root_window,
    text="Dark Mode",
...
color_button = customtkinter.CTkButton(
    color_button_frame,
    text="Select Dark Mode Dot Color (Optional)",
    command=select_dark_mode_color,
)
color_button.grid(row=0, column=0)

label = customtkinter.CTkLabel(
    root_window, text="Select the filetype you'd like to save the image as"
)
label.grid(pady=0)  # Add vertical padding
# Create a dropdown menu with the file types
file_type_dropdown = customtkinter.CTkOptionMenu(
    root_window,
    values=["PNG", "SVG"],
